...
/*   Updated: 2023/12/26 18:55:05 by bvercaem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ms_add_var(t_darray *tokens, t_token *new, char **str, int *qt_or_special)
{
	char	*end;
	char	temp;
	char	**value;
	...
	temp = *end;
	*end = 0;
	value = ms_malloc_getenv(*str + 1, qt_or_special);
	if (!value)
