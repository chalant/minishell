...
int	earley_complete(t_darray *sets, t_ms_grammar *grammar, int j, int state_id)
{
				...
				item->start)))->items->size)
	{
		...
		if (strcmp(next_symbol(grammar, old_item)->name, grammar->rules[item->rule]->name) == 0)
		{
...
int	next_earley_items(t_parsing_data *data, t_earley_set *set, int state_id, int item_idx)
{
	...
	if (symbol->symbol_type == MS_NON_TERMINAL_SYMBOL)
		return (earley_predict(data->earley_sets, data->grammar, symbol, state_id));
		...
		return (earley_complete(data->earley_sets, data->grammar, item_idx, state_id));
	return (0);
}
